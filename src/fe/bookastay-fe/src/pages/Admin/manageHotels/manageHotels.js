...
function ManageHotels() {
  ...
  const fetchHotels = async (page, limit) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:3001/api/hotels/getAll?page=${page}&limit=${limit}`);
      const data = await response.json();

      // Cập nhật state với dữ liệu từ API
      setApplications(data.hotels);
      setTotalPages(data.total_pages);
      setTotalHotels(data.total); // Lấy tổng số người dùng từ API
    } catch (error) {
